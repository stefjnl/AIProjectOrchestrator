    ...
    public class LMStudioClient : BaseAIClientHandler, IAIClient
    {
        ...
        public override async Task<IEnumerable<string>> GetModelsAsync()
        {
            try
            {
                ...
                {
                    _logger.LogWarning("LMStudioClient failed to retrieve models, status: {StatusCode}", response.StatusCode);
            ...
            {
                _logger.LogError(ex, "Error in LMStudioClient.GetModelsAsync for provider {ProviderName}", ProviderName);
                return new List<string> { _settings.DefaultModel };
            }
        }
    }
